...
function showNews($conn, $date_debut, $date_fin, $nom_fil)
{
    ...
    if ($nom_fil == "total") {
        $sql = "SELECT S.id_st , date_abs, nom_tch ,seance, nom_st, prenom_st,heure_debut_abs,heure_fin_abs from teachers as T, etat_absence as E , stagiaire as S, filiere as F
        where T.id_tch = E.id_tch and E.id_st = S.id_st and F.code_fil = S.code_fil  and
          (to_days(date_abs) >= to_days('$debut') AND to_days(date_abs) <= to_days('$date_fin'));";
    } else {
        $sql = "SELECT S.id_st , date_abs, nom_tch ,seance, nom_st, prenom_st,heure_debut_abs,heure_fin_abs from teachers as T, etat_absence as E , stagiaire as S, filiere as F
        where T.id_tch = E.id_tch and E.id_st = S.id_st and F.code_fil = S.code_fil  and
          (to_days(date_abs) >= to_days('$debut') AND to_days(date_abs) <= to_days('$date_fin')) AND nom_fil ='$nom_fil';";
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        <tr>";
        if($date_debut == "all"){
            echo "<th colspan='8'>$nom_fil  cette année</th>";
        }
        else{
            echo "<th colspan='8'>$nom_fil  $debut => $date_fin</th>";
