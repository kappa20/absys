...
-- Retourner le nombre total d'absence d une filiere d'aprés l id d un stagiare
SELECT SUM(heure_absence_st) FROM stagiaire WHERE code_fil = (select code_fil from stagiaire where id_st = 1);

--Retourner le nombre total d'heures non justifier d'un group--
select S.id_st,S.nom_st,S.prenom_st,SUM(((minute(heure_fin_abs) + (hour(heure_fin_abs)*60))-
            (minute(heure_debut_abs) + (hour(heure_debut_abs)*60))) Div 60) AS hours
from etat_absence as E, stagiaire as S where code_fil = (select code_fil from filiere where nom_fil = 'DEV102')
and etat_justif = 'NJ' and S.id_st = E.id_st
group by S.id_st
order by hours desc
